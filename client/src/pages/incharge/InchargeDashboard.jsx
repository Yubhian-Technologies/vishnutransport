          ...
          <>
            ...
            <div className="card">
                  <div className="flex items-center gap-2 mb-4">
                    ...
                    <h2 className="mb-0">My Bus Application</h2>
                  </div>
                  {myApp ? (
                    <div className="flex items-center justify-between">
                      <div>
                        <p className="text-sm text-gray-600">{myApp.routeName}</p>
                        <p className="text-xs text-gray-400 mt-0.5">
                          Fare: {formatCurrency(myApp.fare)} {myApp.concessionReason ? '(50% concession)' : ''}
                        </p>
                      </div>
                      <div className="flex items-center gap-3">
                        <StatusBadge status={myApp.status} />
                        <Link to="/incharge/status" className="text-xs text-primary-600 hover:underline">View →</Link>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-center justify-between">
                      <p className="text-sm text-gray-500">No application submitted yet.</p>
                      <Link to="/incharge/apply" className="btn-primary text-sm px-4 py-2">Apply for Bus</Link>
                    </div>
                  )}
                </div>
